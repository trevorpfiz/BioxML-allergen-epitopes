...
check_arg() {
    ...
    then
        echo -e "${BRed}ENV must be either stage or prod${ColorOff}\n"
        exit 1
    fi
}
...
config_ecr() {
    ...
    docker build --platform linux/amd64 -t $IMAGE:latest -f lambda.Dockerfile .
...
main() {
    ...
    echo -e "${Yellow}If the browser does not show the API documentation properly, refresh after a few seconds.\n"
}
